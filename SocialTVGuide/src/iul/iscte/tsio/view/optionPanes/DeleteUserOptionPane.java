...
public class DeleteUserOptionPane {
	...
	public DeleteUserOptionPane() {
		...
		((JTextComponent) (usernameComboBox.getEditor().getEditorComponent()))
				.getDocument().addDocumentListener(new DocumentListener() {
					...
					public void changedUpdate(DocumentEvent arg0) {
						System.out
								.println("Change update activated, on DeleteUserOptionPane JTextField");
					}
				});
		...
		Object[] buttonsLabels = new Object[] {
				Labels.DELETEUSERBUTTON_DELETEUSER_PANE.getValue(),
				Labels.CANCELBUTTON_DELETEUSER_PANE.getValue() };

		boolean exit = false;
		while (!exit) {
			int result = JOptionPane.showOptionDialog(null, panel,
					Labels.TITLE_DELETEUSER_PANE.getValue(),
			...
			if (result == JOptionPane.OK_OPTION) {
				...
				if (usernames.contains(name)) {
					System.out.print("Deleting user with \"" + name + "\"... ");

					boolean opResult = UserDAOImpl.getInstance().deleteUser(
							UserDAOImpl.getInstance().getUserByName(name));
					...
					if (opResult) {
						String message = String.format(
								Labels.TEXT_SUCCESSDELETINGUSER_DELETEUSER_PANE
										.getValue(), name);
						JOptionPane
								.showMessageDialog(
										null,
										message,
										Labels.TITLE_SUCCESSDELETINGUSER_DELETEUSER_PANE
												.getValue(),
										JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(null,
								Labels.TEXT_ERRORDELETINGUSER_DELETEUSER_PANE
										.getValue(),
								Labels.TITLE_ERRORDELETINGUSER_DELETEUSER_PANE
					...
					JOptionPane.showMessageDialog(null,
							Labels.TEXT_INVALIDDELETINGUSER_DELETEUSER_PANE
									.getValue(),
							Labels.TITLE_INVALIDDELETINGUSER_DELETEUSER_PANE
									.getValue(), JOptionPane.ERROR_MESSAGE);
				}
			} else {
				exit = true;
	...
	private void searchForUser() {
		if (Server.getInstance().isConnected()) {
			...
			try {
				...
				System.err
						.println("NullPointerException on DeleteFriendOptionPane!");
			}
